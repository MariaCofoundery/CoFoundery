...
import { getPublicNetworkProfile, getPublicNetworkProfileListings, publicNetworkPhotoUrl } from "@/features/network/publicNetworkData";
...
export async function generateMetadata({ params }: { params: Promise<{ publicSlug: string }> }): Promise<Metadata> {
  ...
  const profile = await getPublicNetworkProfile(await createClient(), publicSlug).catch(() => null);
  if (!profile) return { title: "Network | CoFoundery", robots: { index: false, follow: false } };
  const canonical = `${getPublicAppOrigin()}/network/p/${profile.public_slug}`;
  return {
    title: `${profile.display_name} – ${profile.headline} | CoFoundery Network`,
...
export default async function PublicNetworkProfilePage({ params }: { params: Promise<{ publicSlug: string }> }) {
  const { publicSlug } = await params;
  const client = await createClient();
  const [t, profile, listings] = await Promise.all([
    getTranslations("network"),
    getPublicNetworkProfile(client, publicSlug).catch(() => null),
    getPublicNetworkProfileListings(client, publicSlug).catch(() => []),
  ]);
  if (!profile) notFound();
  const photo = profile.photo_available ? publicNetworkPhotoUrl("profile", profile.public_slug, profile.updated_at) : null;

  return <PublicNetworkShell><main className="mx-auto max-w-4xl px-5 py-10 md:px-8 md:py-14">
    ...
    <article className="mt-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-9">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
        <PublicNetworkAvatar src={photo} displayName={profile.display_name} className="h-20 w-20" />
