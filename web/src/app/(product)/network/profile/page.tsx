...
import { NetworkVisibilityField } from "@/features/network/NetworkVisibilityField";
...
export default async function NetworkProfilePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const continuation = params.next?.startsWith("/network/l/") && !params.next.startsWith("//") ? params.next : "";
  return <main className="mx-auto max-w-4xl px-5 py-10">
    ...
    <form action={saveNetworkProfileAction} className="mt-6 space-y-6 rounded-3xl border border-slate-200 bg-white p-6">
      ...
      />
      <NetworkVisibilityField initial={profile?.visibility} copy={{
      ...
      {profile?.visibility === "public" && profile.status === "active" ? <Link href={`/network/p/${profile.public_slug}`} className="inline-flex min-h-11 items-center font-semibold text-violet-800 hover:underline">{t("visibility.openPublicPage")}</Link> : null}
