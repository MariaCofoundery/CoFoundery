...
import { getActiveNetworkListings, getIncomingPendingNetworkContactCount, getUnreadNetworkMessageCount } from "@/features/network/networkData";
...
export default async function NetworkPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const { client, user } = await requireNetworkMember(); const [listings, baseProfile, incomingContacts, unreadMessages] = await Promise.all([getActiveNetworkListings(client, filters), getProfileBasicsRow(client, user.id).catch(() => null), getIncomingPendingNetworkContactCount(client, user.id), getUnreadNetworkMessageCount(client)]);
  const networkAttentionCount = getNetworkAttentionCount(incomingContacts, unreadMessages);
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(103,232,249,.13),transparent_30%),linear-gradient(180deg,#fff,#f8fafc)] px-5 py-8 text-slate-950 md:px-8">
    <div className="mx-auto max-w-6xl space-y-6">
      <header className={card}><p className="text-xs font-semibold uppercase tracking-[.2em] text-slate-500">{t("eyebrow")}</p><h1 className="mt-3 text-4xl font-semibold tracking-[-.04em]">{t("title")}</h1><p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">{t("subtitle")}</p>
        ...
        <nav className="mt-5 flex flex-wrap gap-4 text-sm"><Link href="/network/my" className="font-semibold text-slate-700 underline-offset-4 hover:underline">{t("actions.my")}</Link><Link href="/network/contacts" className="inline-flex items-center gap-2 font-semibold text-slate-700 underline-offset-4 hover:underline">{t("actions.contacts")}{networkAttentionCount > 0 ? <span aria-label={t("messages.attentionCount", { count: networkAttentionCount })} className="inline-flex min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 py-0.5 text-[.68rem] font-bold leading-none text-white">{Math.min(networkAttentionCount, 99)}</span> : null}</Link><Link href="/network/profile" className="font-semibold text-slate-700 underline-offset-4 hover:underline">{t("actions.profile")}</Link></nav>
      </header>
