import Link from "next/link"; import { notFound } from "next/navigation"; import { getTranslations } from "next-intl/server";
...
export default async function ListingDetail({ params }: { params: Promise<{listingId:string}> }) {
  const { listingId } = await params; const t = await getTranslations("network"); const { client, user } = await requireNetworkMember(`/network/listings/${listingId}`); const listing = await getNetworkListing(client, listingId); if (!listing) notFound();
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-9"><article className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-9">
    ...
    <section className="mt-7"><h2 className="font-semibold">{t("detail.framework")}</h2><p className="mt-2 text-slate-600">{[listing.location_region, listing.remote_mode ? t(`remote.${listing.remote_mode}`) : null, listing.timeframe, listing.venture_stage ? t(`stages.${listing.venture_stage}`) : null].filter(Boolean).join(" · ") || t("detail.noFramework")}</p></section>
