...
export default async function ReadMyMindRoundPage({ params, searchParams }: { params: Promise<{ teamId: string; roundId: string }>; searchParams: Promise<{ prompt?: string; result?: string; intro?: string }> }) {
  ...
  if (round.status === "forming" && round.ownParticipantState === "pending" && !round.handoffReadyAt) return shell(
    <section className="rmm-enter mt-6 overflow-hidden rounded-[28px] border border-violet-200/80 bg-gradient-to-br from-white via-violet-50/50 to-amber-50/60 p-6 shadow-[0_20px_50px_rgba(76,29,149,0.08)]"><h2 className="text-xl font-semibold">{t("partnerWaitingTitle", { name: partnerName })}</h2><p className="mt-3 text-sm leading-7 text-slate-600">{t("partnerWaitingText", { name: partnerName })}</p><ReadMyMindHandoffVisual selfLabel={t("self")} partnerName={partnerName} /></section>
  );
  ...
  if (round.status === "forming" && round.handoffReadyAt) return shell(
    <section className="rmm-enter mt-6 overflow-hidden rounded-[28px] border border-violet-200/80 bg-gradient-to-br from-white via-violet-50/50 to-amber-50/60 p-6 shadow-[0_20px_50px_rgba(76,29,149,0.08)]"><h2 className="text-xl font-semibold">{t("creatorCompleteTitle")}</h2><p className="mt-3 text-sm leading-7 text-slate-600">{t("creatorCompleteText", { name: partnerName })}</p><p className="mt-2 text-sm text-slate-500">{t("creatorCompleteHint", { name: partnerName })}</p><ReadMyMindHandoffVisual selfLabel={t("self")} partnerName={partnerName} /><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><Link href={`/teams/${encodeURIComponent(teamId)}/collaboration-lab/read-my-mind`} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-violet-700 px-5 py-3 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">{t("chooseAnotherPack")}</Link>{round.handoffEmailClaimedAt === null ? <form action={notifyReadMyMindHandoffsAction.bind(null, teamId)}><button type="submit" className="inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-violet-200 bg-white px-5 py-3 text-sm font-semibold text-violet-800 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">{t("notifyPartner", { name: partnerName })}</button></form> : null}</div><aside className="mt-8 border-t border-slate-200 pt-5" aria-labelledby="rmm-more-options"><h3 id="rmm-more-options" className="text-sm font-semibold text-slate-700">{t("moreOptions")}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{t("discardHint")}</p><div className="mt-3"><ReadMyMindEndControl action={abandon} label={t("discard")} title={t("discardTitle")} confirmation={t("discardConfirm", { name: partnerName })} note={round.handoffEmailClaimedAt ? t("discardEmailNote", { name: partnerName }) : undefined} cancel={t("cancel")} variant="danger" /></div></aside></section>
  );
