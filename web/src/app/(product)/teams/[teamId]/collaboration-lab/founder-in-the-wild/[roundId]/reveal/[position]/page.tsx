...
export default async function FounderInTheWildPromptReveal({ params }: { params: Promise<{ teamId: string; roundId: string; position: string }> }) {
  const { teamId, roundId, position: raw } = await params; const position = Number(raw); if (!Number.isInteger(position) || position < 0 || position > 4) notFound();
  const base = `/teams/${encodeURIComponent(teamId)}/collaboration-lab/founder-in-the-wild/${encodeURIComponent(roundId)}`; const supabase = await createClient(); const { data: { user } } = await supabase.auth.getUser(); if (!user) redirect(`/login?next=${encodeURIComponent(`${base}/reveal/${position}`)}`);
  const team = await getFounderInTheWildTeam(teamId, user.id, supabase); if (!team) notFound(); const round = await getFounderInTheWildRound(team, roundId, user.id, supabase); const prompt = round?.prompts.find((entry) => entry.position === position); if (!round || !prompt) notFound(); if (!round.wholeRoundAnswerComplete) redirect(base);
  const [t, rawLocale] = await Promise.all([getTranslations("founderInTheWild.reveal"), getLocale()]); const locale = normalizeLocale(rawLocale); const partnerName = round.partner.displayName ?? (locale === "de" ? "dein Co-Founder" : "your co-founder"); const opened = round.openedPromptPositions.includes(position); const data = opened ? await getOpenedFounderInTheWildReveal({ team, roundId, position, userId: user.id, client: supabase }) : null;
  if (!opened) return <main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 sm:py-10"><Link href={`${base}/reveal`} className="text-sm text-slate-600 underline">{t("overview")}</Link><section className="mt-6 rounded-[30px] border border-violet-200 bg-gradient-to-br from-violet-50 to-amber-50 p-8 text-center"><h1 className="text-3xl font-semibold">{prompt.content.title[locale]}</h1><p className="mt-3 text-sm text-slate-600">{t("sealed")}</p><form action={openFounderInTheWildRevealAction.bind(null, teamId, roundId, position)} className="mt-6"><button className="min-h-12 rounded-xl bg-violet-700 px-6 py-3 text-sm font-semibold text-white">{t("open")}</button></form></section></main>;
