...
import { reuseExistingProfileAction, saveConnectProfileAction } from "@/features/connect/connectActions";
import { ConnectSubmitButton } from "@/features/connect/ConnectSubmitButton";
import { CONNECT_REMOTE_MODES, CONNECT_ROLES } from "@/features/connect/connectTypes";
...
export default async function ConnectProfilePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [profile, baseProfile, params] = await Promise.all([getOwnConnectProfile(client, user.id), getProfileBasicsRow(client, user.id).catch(() => null), searchParams]);
  ...
  return <main className="mx-auto max-w-4xl px-5 py-10">
    ...
    <form action={saveConnectProfileAction} className="mt-6 space-y-6 rounded-3xl border border-slate-200 bg-white p-6">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium">{t("profile.remote")}<select name="remote_mode" defaultValue={profile?.remote_mode || ""} className={field}><option value="">{t("form.optional")}</option>{CONNECT_REMOTE_MODES.map((value) => <option key={value} value={value}>{t(`remote.${value}`)}</option>)}</select></label>
      </div>
      <ConnectPhotoField
