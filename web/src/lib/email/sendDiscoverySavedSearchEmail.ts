...
import { getDiscoverySavedSearchEmailCopy, getEmailPrivacyUrl } from "@/features/email/emailMessages";
import type { DiscoverySavedSearchReason } from "@/features/discovery/discoverySavedSearchMatching";
...
function buildFromAddress() {
  ...
  return `${process.env.RESEND_FROM_NAME?.trim() || "Cofoundery"} <${fromEmail}>`;
}

function describeReasons(reasons: DiscoverySavedSearchReason[], locale: AppLocale) {
  const labels: Record<DiscoverySavedSearchReason["key"], string> =
    locale === "en"
      ? { query: "Search term", role: "Role", expertise: "Expertise", industry: "Industry",
          location: "Place", remote: "Way of working", capability: "Capability" }
      : { query: "Suchbegriff", role: "Rolle", expertise: "Expertise", industry: "Branche",
          location: "Ort", remote: "Arbeitsweise", capability: "Fähigkeit" };
  return reasons.map((reason) => `${labels[reason.key]}: ${reason.value}`);
}

function buildHtmlBody(params: Params, locale: AppLocale) {
  const copy = getDiscoverySavedSearchEmailCopy(locale, {
      ...
      <tr><td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border:1px solid #e2e8f0;border-radius:24px;overflow:hidden;">
          ...
          <tr><td style="padding:24px 32px 32px;">
            ...
            <ul style="margin:0 0 18px;padding-left:22px;color:#334155;">${copy.reasons.map((reason) => `<li style="margin:5px 0;font-size:15px;line-height:23px;">${escapeHtml(reason)}</li>`).join("")}</ul>
            ${copy.alignmentNote ? `<p style="margin:0 0 20px;font-size:14px;line-height:23px;color:#475569;">${escapeHtml(copy.alignmentNote)}</p>` : ""}
...
export async function sendDiscoverySavedSearchEmail(params: Params): Promise<Result> {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from, to: [params.recipientEmail],
      reply_to: process.env.RESEND_REPLY_TO_EMAIL?.trim() || undefined,
      subject: copy.subject, html: buildHtmlBody(params, locale),
    }),
  });
