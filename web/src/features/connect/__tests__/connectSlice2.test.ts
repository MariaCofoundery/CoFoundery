...
import { normalizeConnectContactMessage } from "@/features/connect/connectValidation";
...
test("request creation validates bounded plain text and uses pending feedback", () => {
  assert.equal(normalizeConnectContactMessage("  A useful short introduction.  "), "A useful short introduction.");
  assert.equal(normalizeConnectContactMessage("Too short"), null);
  assert.equal(normalizeConnectContactMessage("x".repeat(501)), null);
  const create = source("src/app/(product)/connect/listings/[listingId]/contact/page.tsx");
  ...
  assert.match(create, /ConnectSubmitButton/);
...
test("listing detail shows the correct contact state and never offers owners a request CTA", () => {
  const detail = source("src/app/(product)/connect/listings/[listingId]/page.tsx");
...
test("contacts keep pending lifecycle actions separate from accepted conversations", () => {
  ...
  const controls = source("src/features/connect/ConnectContactActions.tsx");
...
test("incoming pending requests become NEEDS_YOU tasks while resolved and outgoing requests do not", () => {
  const incoming = buildFounderDashboardTasks({
    ...dashboardSignals(),
    connectContacts: [{ id: "request-1", senderLabel: "Ada", listingTitle: "B2B Sales", updatedAt: "2026-09-03T11:00:00.000Z" }],
  });
  assert.deepEqual(
    incoming.map(({ type, kind, href, personLabel, contextLabel }) => ({ type, kind, href, personLabel, contextLabel })),
    [{ type: "network_contact", kind: "NEEDS_YOU", href: "/connect/contacts", personLabel: "Ada", contextLabel: "B2B Sales" }]
  );
  assert.equal(buildFounderDashboardTasks({ ...dashboardSignals(), connectContacts: [] }).length, 0);
...
test("Connect-only and Advisor contact access remains capability-based", () => {
  const sql = source("../supabase/migrations/20260903210000_create_network_contact_requests.sql");
...
test("messages stay outside listing and broad Connect payloads", () => {
  const data = source("src/features/connect/connectData.ts");
  const card = source("src/features/connect/ConnectListingCard.tsx");
...
test("Connect navigation exposes contacts without creating a global social graph", () => {
  const home = source("src/app/(product)/connect/page.tsx");
  const contacts = source("src/app/(product)/connect/contacts/page.tsx");
  assert.match(home, /href="\/connect\/contacts"/);
  assert.match(contacts, /href="\/connect"/);
...
test("Slice 2 adds neither contact email automation nor private contact-channel publication", () => {
  const changedSources = [
    ...
    source("src/app/(product)/connect/contacts/page.tsx"),
