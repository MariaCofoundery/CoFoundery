...
test("Connect photos use an explicit private projection and never imply public publication", () => {
...
test("photo removal and replacement clean only owner Connect objects", () => {
  ...
  assert.match(actions, /storage\.from\(CONNECT_PHOTO_BUCKET\)\.remove/);
});

test("authenticated photo delivery is proxied and fails closed", () => {
  const route = source("src/app/api/connect/photos/[userId]/route.ts");
...
test("Connect identity photos appear without dominating listings, contacts, or chat", () => {
