...
import { categorySupportsRemoteMode, coFounderBridgeHref, CONNECT_CATEGORIES, CONNECT_GEOGRAPHIC_SCOPES, CONNECT_ROLES } from "@/features/connect/connectTypes";
import { formatConnectContentTimeframe, normalizeConnectLocations } from "@/features/connect/connectPresentation";
import { ConnectValidationError, listingPublishable, parseCommaSeparatedList, parseConnectListing, parseConnectProfile, profilePublishable } from "@/features/connect/connectValidation";
...
test("Connect categories deliberately exclude co-founder listings", () => {
  assert.deepEqual(CONNECT_CATEGORIES, ["expertise", "cooperation", "investment", "sparring", "succession"]);
  assert.ok(!CONNECT_CATEGORIES.includes("cofounder" as never));
});
...
test("listing input is bounded and supports seeking and offering", () => {
  for (const direction of ["seeking", "offering"]) {
    ...
    const parsed = parseConnectListing(form);
    assert.equal(parsed.direction, direction); assert.equal(parsed.topics.length, 8); assert.ok(listingPublishable(parsed));
  }
});

test("Connect home preserves seeking and offering intent through the create flow", () => {
  const home = readFileSync("src/app/(product)/connect/page.tsx", "utf8");
  const create = readFileSync("src/app/(product)/connect/listings/new/page.tsx", "utf8");
  assert.match(home, /new\?direction=seeking/); assert.match(home, /new\?direction=offering/);
  assert.match(create, /isOneOf\(CONNECT_DIRECTIONS, params\.direction\)/);
  assert.match(create, /direction=\$\{direction \|\| "seeking"\}/);
  assert.match(readFileSync("src/features/connect/ConnectListingForm.tsx", "utf8"), /name="direction"/);
});
...
test("locations trim and deduplicate without silently exceeding the maximum", () => {
  ...
  assert.throws(() => parseCommaSeparatedList("Berlin,Hamburg,Köln,München", 3, "too_many_locations"), (error) => error instanceof ConnectValidationError && error.code === "too_many_locations");
});

test("geographic scope and category-aware fields remain explicit", () => {
  for (const scope of CONNECT_GEOGRAPHIC_SCOPES) {
    ...
    const parsed = parseConnectListing(form); assert.equal(parsed.geographic_scope, scope); assert.equal(parsed.remote_mode, null);
  }
...
test("content dates validate independently from listing expiry and format without false precision", () => {
  ...
  assert.ok(listingPublishable({ ...parseConnectListing(valid), title: "Valid title", summary: "A sufficiently complete summary." }));
  ...
  assert.equal(listingPublishable({ ...parseConnectListing(invalid), title: "Valid title", summary: "A sufficiently complete summary." }), false);
  assert.equal(formatConnectContentTimeframe("2026-10-01", null, "de", { from: "Ab", until: "Bis" }), "Ab Oktober 2026");
  assert.equal(formatConnectContentTimeframe(null, "2026-12-31", "en", { from: "From", until: "Until" }), "Until December 2026");
  assert.equal(formatConnectContentTimeframe("2026-10-01", "2026-12-31", "de", { from: "Ab", until: "Bis" }), "Oktober–Dezember 2026");
  assert.equal(formatConnectContentTimeframe(null, null, "de", { from: "Ab", until: "Bis" }), null);
});
...
test("home layout and empty state support low-liquidity counts without ranking", () => {
  ...
  const data = readFileSync("src/features/connect/connectData.ts", "utf8");
...
test("initial Connect render accepts membership without an owner Connect profile and zero listings", () => {
  const page = readFileSync("src/app/(product)/connect/page.tsx", "utf8");
  const data = readFileSync("src/features/connect/connectData.ts", "utf8");
  const layout = readFileSync("src/app/layout.tsx", "utf8");
  assert.match(data, /return \(data \?\? \[\]\) as ConnectListing\[\]/);
...
test("co-founder selection bridges to Discovery and never reaches the listing editor", () => {
  const page = readFileSync("src/app/(product)/connect/listings/new/page.tsx", "utf8");
...
test("product entry keeps Founder, Advisor, Connect-only, and unsupported access separate", () => {
  ...
  const connectOnly = { ...founder, hasFounder: false };
  const unsupported = { ...connectOnly, hasConnect: false };
  ...
  assert.equal(resolveProductEntryPath("/dashboard", connectOnly, "/welcome"), "/connect");
  assert.equal(resolveProductEntryPath("/connect/my", connectOnly, "/welcome"), "/connect/my");
...
test("German and English Connect messages have exact key parity", () => {
  const de = JSON.parse(readFileSync("messages/de/connect.json", "utf8"));
  const en = JSON.parse(readFileSync("messages/en/connect.json", "utf8"));
...
test("legacy-shaped listing locations cannot crash card or detail rendering", () => {
  assert.deepEqual(normalizeConnectLocations(undefined), []);
  assert.deepEqual(normalizeConnectLocations(null), []);
  assert.deepEqual(normalizeConnectLocations(["Berlin"]), ["Berlin"]);
  for (const path of ["src/features/connect/ConnectListingCard.tsx", "src/app/(product)/connect/listings/[listingId]/page.tsx"]) {
    const renderSource = readFileSync(path, "utf8");
    assert.match(renderSource, /normalizeConnectLocations\(listing\.locations\)/);
...
test("Connect-only account deletion is exposed through shared Account settings", () => {
  const shell = readFileSync("src/features/navigation/ProductShell.tsx", "utf8");
