...
export function ConnectSafetyActions({ otherUserId, contactRequestId, returnTo, blockedByMe, interactionBlocked, copy }: {
  ...
  return <div className="mt-4 border-t border-slate-100 pt-4">
    ...
    <div className="mt-2 flex flex-wrap gap-3">
      {blockedByMe ? <form action={unblockConnectUserAction}>
        <input type="hidden" name="other_user_id" value={otherUserId} /><input type="hidden" name="return_to" value={returnTo} />
        <ConnectSubmitButton label={copy.unblock} pendingLabel={copy.unblocking} className="min-h-11 text-sm font-semibold text-slate-600 underline-offset-4 hover:underline" />
      </form> : !interactionBlocked ? <form action={blockConnectUserAction} onSubmit={(event) => { if (!window.confirm(copy.blockConfirm)) event.preventDefault(); }}>
        <input type="hidden" name="other_user_id" value={otherUserId} /><input type="hidden" name="return_to" value={returnTo} />
      ...
      <details className="w-full sm:w-auto">
        ...
        <form action={reportConnectInteractionAction} className="mt-2 w-full space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:w-96">
          ...
          <ConnectSubmitButton label={copy.reportSubmit} pendingLabel={copy.reporting} className="min-h-11 rounded-full bg-slate-900 px-5 text-sm font-semibold text-white" />
        </form>
      </details>
    </div>
  </div>;
