...
export function PublicNetworkShell({ children }: { children: React.ReactNode }) {
...
export function PublicNetworkAvatar({ src, displayName, className = "h-16 w-16" }: { src: string | null; displayName: string; className?: string }) {
  if (src) {
    // eslint-disable-next-line @next/next/no-img-element
