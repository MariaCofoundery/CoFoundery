...
import { saveDisplayName } from "@/features/questionnaire/actions";
...
export function DisplayNameStep({ sessionId, defaultLabel = "Person A" }: Props) {
  const [value, setValue] = useState("");
  const [saving, setSaving] = useState(false);

  const onSubmit = async () => {
    setSaving(true);
    await saveDisplayName(sessionId, value.trim() ? value.trim() : defaultLabel);
    setSaving(false);
