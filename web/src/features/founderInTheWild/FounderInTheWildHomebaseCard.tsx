...
export async function FounderInTheWildHomebaseCard({ team, currentUserId }: { team: FounderInTheWildTeam; currentUserId: string }) {
  ...
  const entry = `/teams/${encodeURIComponent(team.id)}/collaboration-lab/founder-in-the-wild`;
  return <section className="rounded-2xl border border-amber-200/80 bg-gradient-to-br from-amber-50/60 via-white to-violet-50/50 p-5 shadow-[0_12px_30px_rgba(76,29,149,0.05)] sm:p-6" aria-labelledby="founder-wild-title">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 id="founder-wild-title" className="text-xl font-semibold text-slate-950">{t("title")}</h2><p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600">{t("description")}</p></div>
    {team.members.length === 2 ? <Link href={round ? `${entry}/${encodeURIComponent(round.id)}` : entry} className="inline-flex min-h-11 shrink-0 items-center justify-center rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">{t(round ? round.ownAnswerComplete ? "status" : "continue" : "action")}</Link> : <p className="text-sm text-slate-500">{t("twoFounders")}</p>}</div>
  </section>;
}
